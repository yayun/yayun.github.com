...
如果 master 崩溃了，我们需要从日志中重建状态，但是 master 从启动到当前时间可能很长时间，所以 master 需要额外的创建一份完成的状态到磁盘。创建 checkpoint 需要花费一些时间，所以 master 为了不妨碍后续日志的修改，在创建 checkpoint 的时候，会先创建一个对当前日志的新的 copy，然后启动一个新的线程对这个 copy 日志做 checkpoint。完成之后会将数据存到本地磁盘和远程备份。（The master switches to a new log file and creates the
...
* 当 master 收到 snapshot 请求后master 会撤回 snapshot 所涉及的所有 chunk 的 lease。之所以撤销 lease 是为了让后续在这些 chunk 上的写请求操作都需要和 master 交互。可以让 master 有机会来创建这些 chunk 的新的 copy。
...
每一个 chunkserver 使用 checksumming 来检测本地数据的损坏。一个 GFS 集群通常会有数百台机器数千块磁盘，读取和写入是遇到损坏是常有的事。我们可以使用其他副本的 chunk 数据来恢复数据，通过跨 chunkserver 间的数据比较来检测故障时不现实的。另外两个副本的数据不一致也是合法的。因此每个 chunkserver 需要根据自己维护的 checksum 来独立的验证自己副本数据的完整性。
